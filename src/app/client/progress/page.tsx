...
import { weightHistory, progressPhotos } from "@/lib/mock-data";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--color-charcoal)]/60">
          Your trends over the last {weightHistory.length} weeks.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        />
        <StatTile label="Since" value={new Date(first.date).toLocaleDateString("en-AU", { day: "numeric", month: "short" })} icon={Calendar} accent="var(--color-clay-deep)" />
      </div>
      ...
      <Card>
        ...
        <p className="mt-3 text-xs text-[var(--color-charcoal)]/45">
          Real photo uploads (stored privately per client) are a near-term build step — these
          are placeholder swatches for now.
        </p>
      </Card>
    </div>
  );
